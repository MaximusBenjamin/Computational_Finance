...
def analyze_cascade_with_dynamic_bailout(G, failing_bank, bailout_fund_limit):
    """
    Enhanced bailout logic: dynamically prioritize banks to minimize overall cascade impact.
    """
    ...
    while processing_queue:
        ...
        # Sort banks by cascade impact (descending) and remaining equity (ascending)
        ...
        for bank, (reduced_equity, _) in sorted_failures:
            if reduced_equity < 0 and bailout_fund > 0:
                # Apply bailout fund dynamically
            ...
            # Update the equity state
            G.nodes[bank]['equity'] = reduced_equity
            
            # If the bank fails, add to the cascade
...
for bank in equity_sheet['Bank']:
    ...
    affected_banks, final_equities, remaining_bailout_fund, critical_banks = analyze_cascade_with_dynamic_bailout(
